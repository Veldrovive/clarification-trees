...
class DialogTreeHarness:
    def __init__(self,
        ...
        clarification_model: ModelInterface,
        clarifying_model: ModelInterface,
        inference_model: ModelInterface
    ):
        self.cfg = cfg
        self.logger = logger
        ...
        if cfg.runtime_meta.git_commit:
            git_matched = check_git_commit(cfg.runtime_meta.git_commit)
            if not git_matched:
                if cfg.runtime_meta.git_strict:
                    raise ValueError("Git commit does not match the required commit.")
                else:
                    print("Warning: Git commit does not match the required commit.")
        else:
            # Add the git commit to the config
            cfg.runtime_meta.git_commit = get_git_commit()
